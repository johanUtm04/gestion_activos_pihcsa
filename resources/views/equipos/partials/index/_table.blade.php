                        ...
                        <tr id="equipo-{{ $equipo->id }}" 
                            ...
                            <td class="text-center" style="vertical-align: middle;">
                                <div class="btn-group shadow-sm">
                                    ...
                                    @can('eliminar-equipo')
                                        {{-- Cambiamos la lógica a Inactivación --}}
                                    <div>
                                    <button type="button" 
                                                class="btn btn-sm btn-default text-secondary btn-inactivar" 
                                                data-id="{{ $equipo->id }}" 
                                                data-nombre="{{ $equipo->tipoActivo?->nombre }} - {{ $equipo->serial }}"
                                                data-label="motivo de la inactivación" 
                                                data-current="{{ $equipo->motivo_inactivacion }}"
                                                data-motivo-input="#motivo_hidden_{{ $equipo->id }}"
                                                onclick="ejecutarInactivacion(this)"
                                                title="Inactivar Activo">
                                            <i class="fas fa-ban"></i>
                                        </button>
                                        <input type="hidden" name="motivo_inactivacion" id="motivo_inactivacion">
                                    <input type="hidden" name="motivo_inactivacion" id="motivo_inactivacion_{{ $equipo->id }}">                                    </div>
                                    @endcan
                                </div>
<!-- 
                                {{-- Badge de estado para confirmar visualmente --}}
                                @if($equipo->status == 'inactivo')
                                    <div class="mt-1">
